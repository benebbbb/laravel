...
<div class="d-flex justify-content-between align-items-center mb-3">
    <h5 class="fw-bold mb-0"><i class="bi bi-journal-medical me-2 text-primary"></i>My Medicine List</h5>
                        ...
                        <tr class="{{ $isExpired ? 'table-danger' : ($isNear ? 'table-warning' : '') }}">
                            ...
                            <td class="text-center">
                                <button type="button" class="btn btn-sm btn-outline-primary me-1"
                                    onclick="openEditMedicine({{ $medicine->id }},'{{ addslashes($medicine->medicine_name) }}','{{ addslashes($medicine->description) }}','{{ $medicine->category }}',{{ $medicine->quantity }},'{{ $medicine->expiration_date->format('Y-m-d') }}')">
                                ...
                                <form id="delete-form-{{ $medicine->id }}" action="{{ route('medicines.destroy', $medicine) }}" method="POST" class="d-none">
                                    @csrf @method('DELETE')
                                </form>
                            </td>
                        </tr>
                    @empty
                        <tr>
                            <td colspan="6" class="text-center text-muted py-4">
                                <i class="bi bi-inbox fs-4 d-block mb-1"></i> No medicines found.
                            </td>
                        </tr>
                    @endforelse
                </tbody>
...
<div class="d-md-none">
    @forelse($medicines as $medicine)
        @php
            ...
            $isNear = $daysLeft >= 0 && $daysLeft <= 7;
        @endphp
        <div class="card shadow-sm mb-2 {{ $isExpired ? 'border-danger' : ($isNear ? 'border-warning' : '') }}" style="{{ $isExpired ? 'border-left:4px solid #dc3545!important' : ($isNear ? 'border-left:4px solid #ffc107!important' : 'border-left:4px solid #0d6efd!important') }}">
            <div class="card-body p-3">
                <div class="d-flex justify-content-between align-items-start mb-1">
                    <div>
                        <div class="fw-semibold" style="font-size:14px">{{ $medicine->medicine_name }}</div>
                        <span class="badge bg-secondary" style="font-size:10px">{{ $medicine->category }}</span>
                        @if($isExpired)
                            <span class="badge bg-danger ms-1" style="font-size:10px">Expired</span>
                        @elseif($isNear)
                            <span class="badge bg-warning text-dark ms-1" style="font-size:10px">Expiring Soon</span>
                        @endif
                    </div>
                ...
                <div class="d-flex gap-3 mt-2" style="font-size:12px;color:#6c757d">
                    <span><i class="bi bi-box me-1"></i>Qty: {{ $medicine->quantity }}</span>
                    <span><i class="bi bi-calendar3 me-1"></i>{{ $medicine->expiration_date->format('M d, Y') }}</span>
                </div>
            </div>
        </div>
        <form id="delete-form-{{ $medicine->id }}" action="{{ route('medicines.destroy', $medicine) }}" method="POST" class="d-none">
            ...
            <form method="POST" action="{{ route('medicines.store') }}">
                ...
                <div class="modal-body">
                    ...
                    <div class="mb-3">
                        ...
                        <input type="text" name="medicine_name" class="form-control @error('medicine_name') is-invalid @enderror" value="{{ old('medicine_name') }}" required>
                    ...
                    <div class="mb-3">
                        ...
                        <input type="number" name="quantity" min="0" class="form-control @error('quantity') is-invalid @enderror" value="{{ old('quantity') }}" required>
                        @error('quantity') <div class="invalid-feedback">{{ $message }}</div> @enderror
                    </div>
                    <div class="mb-1">
                        <label class="form-label">Expiration Date <span class="text-danger">*</span></label>
                        <input type="date" name="expiration_date" class="form-control @error('expiration_date') is-invalid @enderror" value="{{ old('expiration_date') }}" required>
...
<div class="modal fade" id="deleteModal" tabindex="-1">
    <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content">
            ...
            <div class="modal-body">Are you sure you want to delete <strong id="deleteMedicineName"></strong>?</div>
